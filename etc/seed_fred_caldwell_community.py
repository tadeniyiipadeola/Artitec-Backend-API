#!/usr/bin/env python3
"""
Seed script for Fred Caldwell's Community Profile
...
async def create_fred_caldwell_community():
    ...
    async for db in get_db():
        try:
            ...
            fred_user = result.scalar_one_or_none()
            ...
            if fred_user:
                ...
                role_result = await db.execute(
                    select(Role).where(Role.key == "community_admin")
                )
            ...
            result = await db.execute(
                select(CommunityAdminProfile).where(
                    CommunityAdminProfile.user_id == fred_user.id
                )
            )
            existing_admin_profile = result.scalar_one_or_none()
            ...
            if existing_admin_profile:
                ...
                result = await db.execute(
                    select(Community).where(Community.id == existing_admin_profile.community_id)
                )
...
async def main():
    ...
    try:
        result = await create_fred_caldwell_community()
        return result
    except Exception as e:
        print(f"\n❌ Script failed: {str(e)}")
        import traceback
